...
function Game(player1, player2) {
  ...
  const reset = () => {
    gridArray.forEach((row, i) => row.fill(0));
    gameIsOver = false;
    currentMarker = firstPlayer.getPlayerMarker();
...
startBtn.addEventListener("click", (event) => {
  ...
  const firstPlayerMarker = document.querySelector("#firstPlayerMarker");

  const secondPlayerName = document.querySelector("#secondPlayerName");
  const secondPlayerMarker = document.querySelector("#secondPlayerMarker");

  if (firstPlayerName.value != "" && secondPlayerName.value != "") {
    initialModal.close();

    let firstPlayerMarkerString = firstPlayerMarker.value;
    let secondPlayerMarkerString = secondPlayerMarker.value;

    if (firstPlayerMarker.value == "") {
      firstPlayerMarkerString = "X";
    }

    if (secondPlayerMarker.value == "") {
      secondPlayerMarkerString = "O";
    }

    const player1 = Player(firstPlayerMarkerString, firstPlayerName.value);
    const player2 = Player(secondPlayerMarkerString, secondPlayerName.value);
